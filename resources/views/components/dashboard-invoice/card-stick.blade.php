...
@php
    ...
    $chartData = $dataDocuments->map(function ($doc) {
        return [
            'month' => date('M', strtotime($doc->period . '-01')), // Ambil bulan
            'year' => date('Y', strtotime($doc->period . '-01')),  // Ambil tahun
            'total' => $doc->total,
        ];
    });
    ...
    <div class="px-5 pt-5">
        ...
        <div class="pt-6 px-2 pb-0">
            <div id="bar-chart" class="w-full min-h-[300px] flex items-center justify-center">
                <p id="no-data-message" class="text-gray-500 dark:text-gray-400 hidden">Belum ada data tagihan.</p>
            </div>
        </div>
    </div>
</div>
...
@php
    // Pastikan hanya mengambil data dengan total lebih dari 0
    $filteredData = $dataDocuments->where('total', '>', 0);

    // Ubah data menjadi JSON untuk JavaScript
    $chartDataJson = json_encode($filteredData);
@endphp
...
<script>
    ...
    function initializeChart1(themeMode1) {
        ...
        // Ambil data dari Laravel yang dikirim ke Blade sebagai JSON
        let chartData = JSON.parse('{!! $chartDataJson !!}');

        // Jika data kosong, tampilkan pesan "Belum ada data tagihan."
        if (!Array.isArray(chartData) || chartData.length === 0) {
            document.getElementById("no-data-message").classList.remove("hidden");
            return;
        }

        // Ambil daftar bulan dan total sesuai data
    ...
    const currentTheme1 = document.documentElement.classList.contains('dark') ? 'dark' : 'light';
    initializeChart1(currentTheme1);
